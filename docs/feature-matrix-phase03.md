...
| 2 | **MR** | Intent Tag Vocabulary, Capture & Ranking Read Path | — | 1 | 🟡 In progress (6 of 7 rows ✅; MR-3's browser-level UI verification remains open) |
...
> **ExecPlan (2026-08-06):** [20260806-mr-intent-tag-capture-and-ranking.md](plans/20260806-mr-intent-tag-capture-and-ranking.md). Authored 2026-08-06; MR-1…MR-7 → 🟡 In progress. Milestones A, B, and C all complete as of 2026-08-08; MR-1, MR-2, MR-4, MR-5, MR-6, MR-7 → ✅ Implemented. MR-3 stays 🟡 In progress — its API half is fully verified end to end, but the Flutter capture UI was verified by `flutter analyze`/`flutter test` only, not by a browser-driven click-through (this repo has no browser-automation tooling), so the milestone's own C4 acceptance bar is not fully closed. See MR-3's row for the precise gap.

| ID | Feature | Status | Description | Req ref |
|---|---|---|---|---|
| MR-1 | Intent + mood vocabulary value objects | ✅ Implemented | `IntentTag` (6 closed values, **each carrying a static arousal coordinate**) and `MoodTag` (valence × arousal + `SHAREABLE`/`PRIVATE_ONLY` sensitivity) in a new `fastapi-service/app/domain/shared/affective.py` — **not** in `domain/closet/`, because MS-1 needs the same vocabulary on `UserPreference` in `domain/styling/` and the two bounded contexts do not import each other. Vocabulary is versioned so labels can change without migrating rows. Stored value is the enum name, never a display string. **Authored Japanese-first**; a weaker English label is compensated by the caption, not by changing the Japanese. Per-locale vocabularies are prohibited — the enum value means the same thing in every locale. Verified: unit tests for the vocabulary's validation rules pass, and the live shared-closet proof (MR-6) exercised the vocabulary end to end against a real Elasticsearch index without it being questioned across two further milestones. | §1.1, ADL-038, ADL-040, ADL-049 |
| MR-2 | `ClothingItem.intent_tags` field + aggregate behavior | ✅ Implemented | Dedicated `intent_tags: List[IntentTag]` on the aggregate (`aggregates.py:16`) with a setter, **not** folded into the existing free-form `tags`. 0–3 values, opt-in; an item with none behaves exactly as today. `set_intent_tags` rejecting >3 values and rejecting duplicates both independently verified passing (Milestone A evidence). | §1.2, ADL-039 |
| MR-3 | Capture UI + metadata API | 🟡 In progress | `UpdateClosetItemMetadata` and the closet `PATCH` route accept `intentTags`; Flutter closet edit dialog gains a localized multi-select, offered once at upload completion and skippable. No new Gemini call — intent is user input, not inference. **API half (C1) fully verified**: a live, non-mocked round trip confirmed `intentTags` PATCHed through the real running stack lands in both Firestore and Elasticsearch, clearing to `[]` leaves the item healthy, and an unknown enum value is rejected with 400 end-to-end. **UI half (C2) built and unit-verified but not click-verified**: `flutter analyze` clean and `flutter test` green (67 passed, including 6 new cases for the 0–3 cap, skip path, and `ClosetItem.fromFirestore` parsing), but the plan's own C4 acceptance ("against `make dev`, in both 日本語 and English: tag an item, reload the page and confirm the tags persisted") was exercised only at the API level, not through the rendered Flutter UI — this repo has no Playwright/Selenium/CDP browser-automation tooling, confirmed by direct search. The chips, captions, and cap-reached hint are therefore **UNPROVEN at the pixel/click level**: their ja/en text was confirmed only by reading the generated ARB output, not by observing them rendered in a browser. Status stays 🟡 pending a browser-capable verification pass. | §1.2 |
| MR-4 | Elasticsearch mapping + indexing | ✅ Implemented | `intentTags` declared `keyword` in the canonical mapping (`fastapi-service/app/adapters/elasticsearch_embedding_repo.py`) alongside `tags`/`category`/`colors`; written on index and update. Existing documents without the field stay valid; no shared-closet reindex required. The live mapping check the plan's own A6 required (`curl .../_mapping` showing `intentTags` as `keyword`, not dynamically-mapped `text`) was originally UNPROVEN for lack of a reachable Docker daemon, then independently confirmed as a side effect of Milestone B's live verification run: `"intentTags":{"type":"keyword"}` on the persistent index. | §1.3 |
...
| MR-7 | Tests + localization | ✅ Implemented | `fastapi-service` pytest, `adk-agent-service` pytest, `flutter analyze`/`flutter test` green; ja/en ARB keys for every vocabulary label and caption. All confirmed by independent reproduction: `adk-agent-service` pytest 75 passed; `fastapi-service` pytest 125 passed, 0 skipped; `flutter analyze` clean; `flutter test` 67 passed; ja/en ARB keys present for all six `IntentTag` labels plus captions, section title, and skip/cap-reached strings. This row covers the test suites and localization keys only — it does not by itself claim the rendered Flutter UI was browser-verified (see MR-3). | §1.1, §1.6 |
...
| Status | Rows |
|---|---|
| ✅ Implemented | 6 (MR-1, MR-2, MR-4, MR-5, MR-6, MR-7) |
| 🟡 In progress | 1 (MR-3) |
