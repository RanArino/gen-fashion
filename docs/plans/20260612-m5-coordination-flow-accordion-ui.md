...
- Observation: Local Vertex/service-account runs need separate project IDs for Firebase Auth emulator and Google Cloud runtime.
  Evidence: With `.env` setting `GOOGLE_CLOUD_PROJECT=your-project-id`, Firebase Admin initially rejected Auth emulator tokens with audience `gen-fashion-local`. FastAPI now uses `FIREBASE_PROJECT_ID`/`auth_project_id` for token verification while Firestore/Vertex continue using `GOOGLE_CLOUD_PROJECT`.
