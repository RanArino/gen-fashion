...
- [x] (2026-07-11) MO-6 — Manual visual check run live against a real `make dev` stack + live Vertex AI/Rakuten credentials (`scripts/mo6_scene_aware_visual_check.py`, session `a23e65b0-4561-45e4-9bbe-eb2047347231`). PASS — see Outcomes for the observed per-input-type result.
...
**2026-07-11 — MO-6 run and result: PASS.** Ran a live Assisted Coordinate session against a real `make dev` stack with live Vertex AI + Rakuten credentials (new helper script `scripts/mo6_scene_aware_visual_check.py`, session `a23e65b0-4561-45e4-9bbe-eb2047347231`, user `UA5moSO6KqaHAw0UcRm0B2d09JOp`). Rather than the three hand-crafted `coord.jpg`-style inputs (not available as separate raw files — `coord.jpg` is itself a *result* screenshot, per the script's docstring and the task that ran it), the session's own proposed candidates were used, since a real Rakuten search naturally returns the same mix of problematic photo types. Three Rakuten candidates were selected alongside the synthetic blue-shirt anchor, one per outfit slot for maximum photo-type variety:

- `rakuten:motemi:10000465` (white pants, "bottom") — a person-wearing-it photo (cropped torso, different model than the one in the generated result), with the model also holding an unrelated black bag, plus a "TORAY" brand tag, a Japan-flag icon, Japanese marketing text ("透けにくい 大人の白パンツ"), a "第1位" ranking badge, and a 4-swatch color grid overlaid on the photo. This is the closest analog to `coord.jpg`'s "person wearing it on the street" and "hand holding an item amid props" failure categories combined into one image.
- `rakuten:alpen:10409627` (white sneakers, "shoes") — a plain single-product studio shot, analogous to `coord.jpg`'s "mostly fine" plain-garment category (a control case).
- `rakuten:style-on-bag:10002419` (canvas tote bag, "bag") — a staged lifestyle/product photo with a floral/book background, an inset diagram, a "第1位" badge, brand text overlays, and a color-swatch strip along the bottom — analogous to `coord.jpg`'s "unrelated props in frame" category.

The generated coordinate image (`http://localhost:9000/gen-fashion-images/UA5moSO6KqaHAw0UcRm0B2d09JOp/coordinates/693217e9d3874e6c86fdf0199f933597.jpg`, saved locally at `/tmp/mo6-evidence-run2/generated_coordinate.jpg`) shows one freshly-synthesized model wearing the blue T-shirt (from the anchor), the white pants, and the white sneakers, carrying the cream/pink tote — each item's color/pattern/shape correctly reproduced. None of the source photos' scene elements leaked through: no TORAY tag, no Japanese overlay text, no ranking badges, no flag icon, no color-swatch grids/strips, no floral/book background, and critically no trace of either source photo's own model (face, pose, hairstyle) or the white-pants photo's second (unselected) bag. Per-input-type verdict: plain product shot (shoes) — pass, as expected; person-wearing-it-plus-props-and-text photo (pants) — pass, the strongest evidence the fix works since this is the exact worn-garment failure mode from `coord.jpg`; staged-scene-with-props-and-text photo (bag) — pass. Overall MO-6 verdict: **PASS**. Evidence retained at `/tmp/mo6-evidence-run2/` (`candidates.json`, three `source_*.jpg` reference photos, `generated_coordinate.jpg`, `summary.json`).
