...
- a small number of recommendations, selected by default in the UI
...
- selected by default when candidate has `recommended == true` or when it is an anchor
...
7. Confirm `session.proposed` returns anchor and Rakuten candidates; recommended checks are selected.
