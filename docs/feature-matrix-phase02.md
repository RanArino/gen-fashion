...
| MP-7 | Tests + manual verification | 🟡 In progress | 6 new `coordination_screen_test.dart` cases + new `tab_persistence_test.dart` all pass (`flutter test` 51/51; current audit: 61 passed). Post-deploy Cloud Run CPU-throttling check is done. **Remaining:** local `make dev` manual browser pass. | §5 |
