...
| MD-3 | Compute Engine Elasticsearch node | 🟡 In progress | Provision `e2-medium` VM (no external IP, **30 GB `pd-standard`**, **static internal IP**) in `asia-northeast1-a`; install ES 8.x with `xpack.security` enabled + API key; systemd auto-start; firewall `tcp:9200` from the **Cloud Run Direct VPC egress subnet range** only. **Cost (ADL-023):** `pd-standard` not SSD; stop the VM when idle; optional `Asia/Tokyo` night-stop instance schedule. Completes the GCE half of M1-3. | §9.2, ADL-013, ADL-023 |
