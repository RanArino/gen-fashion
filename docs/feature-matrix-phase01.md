...
| M1-3 | Elasticsearch on Compute Engine PoC | ✅ Implemented | **Re-scoped 2026-06-04 (ExecPlan Decision Log).** M1 was implemented locally first, then the deferred deployment proof closed in MD: Compute Engine ES `gen-fashion-es` runs at static internal IP `10.146.0.2`, Cloud Run uses Direct VPC egress, firewall allows only the private subnet to tcp:9200, and the 2026-07-11 audit found Cloud Run startup logs reporting `Elasticsearch index ready`. The curated dataset did not require a JP analyzer. | §9.2, ADL-013, ADL-023, §17 |
...
| M3-2 | `shared_closet` data populated | ✅ Implemented | Seeds **3 realistic demo closets** (Adult×2 + Child×1, 70 items each = 210), segmented by `kids`. The stable 50-item base is selected first, then each closet appends 10 tops + 10 bottoms; all 210 images are unique. Local ES/Firestore/MinIO contain 70/70/70 and Firestore emulator state is restored from `gen-fashion_firestore-data`. Production vector seed also completed on the GCE ES VM via `run_seed.py --with-embeddings` (`created=209, skipped=1, errors=0`; `clothing_items/_count=210`; 768-dim embeddings verified). | §16, §8.1/§8.2, ADL-010 |
...
>
> **Status audit (2026-07-11):** Local code, deploy scripts, live GCP resources, and production Firestore state were re-checked. MD-2/MD-4/MD-5/MD-7/MD-8/MD-11/MD-13/MD-14 were stale as 🟡 and are now ✅. MD-9 remains 🟡 because the current R2 credentials return `AccessDenied` for `GetBucketCors`, so `https://gen-fashion-app.web.app` CORS cannot be independently confirmed from this repo/operator context.

| ID | Feature | Status | Description | Req ref |
|---|---|---|---|---|
| MD-1 | GCP project & IAM foundation | ✅ Implemented | **2026-06-27 (Milestone A):** project `animation-agent`; enabled core APIs + Firebase Management/Hosting/Identity Toolkit; created `fastapi-sa`/`adk-sa`/`tasks-invoker-sa` with least-privilege roles (+ `fastapi-sa`→`tasks-invoker-sa` `serviceAccountUser`); Firestore Native `asia-northeast1` created with `firestore.rules`+`firestore.indexes.json` deployed; Firebase added, Google sign-in enabled, Web app `gen-fashion-web` registered. | §9.1, §12.1, ADL-012 |
| MD-2 | Secret Manager + config split | ✅ Implemented | All required secrets are consumed through Secret Manager (`ELASTICSEARCH_API_KEY`, R2 keys, `INTERNAL_TASK_SECRET`; ADK also consumes Rakuten secrets) and non-secret Cloud Run config is applied via env vars. 2026-07-11 audit confirmed both Cloud Run services read the expected env/secrets split. | §12.1, §12.2, ADL-012 |
| MD-3 | Compute Engine Elasticsearch node | ✅ Implemented | **2026-06-28 (Milestone B complete):** `gen-fashion-es` (`e2-medium`, `pd-balanced 30GB`, `asia-northeast1-a`); static internal IP `gen-fashion-es-ip` reserved; ES 8.19 installed (security enabled, `discovery.type: single-node`); cluster health `green`; `ELASTICSEARCH_API_KEY` in Secret Manager; firewall `allow-es-from-cloudrun` (subnet CIDR → tcp:9200); external IP removed. `es-night-off` exists but is detached until 2026-08-31 for the public demo window. | §9.2, ADL-013, ADL-023 |
| MD-4 | Cloud Run ↔ ES private connectivity | ✅ Implemented | Both Cloud Run services use Direct VPC egress (`network=default`, `subnet=default`, `private-ranges-only`), the ES VM has no external IP, and firewall `allow-es-from-cloudrun` allows only `10.146.0.0/20` to tcp:9200. 2026-07-11 audit observed the live FastAPI startup log `[startup] Elasticsearch index ready`, confirming Cloud Run can reach ES privately. | §9.2, ADL-013, ADL-023, §17 |
| MD-5 | Artifact Registry images | ✅ Implemented | Artifact Registry Docker repo `gen-fashion` exists in `asia-northeast1`; images for `fastapi-service` and `adk-agent-service` are built/pushed by both the original Cloud Build path and the current GitHub Actions CD path. 2026-07-11 audit confirmed the repo, recent `ci-*` image tags, and keep-3 cleanup policy. | §9.1 |
| MD-6 | Deploy `fastapi-service` (Cloud Run) | ✅ Implemented | Public ingress, min 0 / max 10, 1 GB / 1 CPU / 300 s; env + secrets + `fastapi-sa`; `TASK_QUEUE_MODE=cloud_tasks`; wired to ADK + own worker URL. **2026-06-29 D.5:** timeout raised from 60 s to 300 s to keep the SSE stream alive through the 150 s app stream window. | §9.1, ADL-016 |
| MD-7 | Deploy `adk-agent-service` (Cloud Run) | ✅ Implemented | Private Cloud Run service (`--no-allow-unauthenticated`) with max 5, 2 GB / 1 CPU / 600 s, `GOOGLE_GENAI_USE_VERTEXAI=true`, `GOOGLE_CLOUD_LOCATION=global`, Direct VPC egress, `adk-sa`, and CPU throttling disabled. 2026-07-11 unauthenticated `/health` check returned 403 as expected. | §9.1, ADL-016 |
| MD-8 | Cloud Tasks queue + OIDC hardening | ✅ Implemented | `gen-fashion-embed` queue is RUNNING; `FASTAPI_INTERNAL_BASE_URL` and `ADK_INTERNAL_BASE_URL` are split; `CloudTasksAdapter` attaches an OIDC token; `adk_agent_run.py` uses an identity token for the FastAPI→ADK hop; `require_internal_secret` verifies OIDC bearer email/audience plus the shared secret. 2026-07-11 audit confirmed the queue and live env wiring. | §6.8, §10.3, ADL-024 |
| MD-9 | Production R2 bucket + CORS | 🟡 In progress | **2026-06-27 (Milestone A):** Cloudflare R2 bucket `gen-fashion-images` created (Public access OFF — all access via S3 presigned URLs); initial CORS allowed `GET/PUT/POST` from `https://your-project-id.web.app`, `https://your-project-id.firebaseapp.com`, `https://localhost:8080`; R2 API-token keys stored in Secret Manager (`R2_ACCESS_KEY_ID`/`R2_SECRET_ACCESS_KEY`). **2026-06-29 D.5:** new Hosting fallback is `https://gen-fashion-app.web.app`; adding that origin to R2 CORS was still pending because the current R2 API keys returned `AccessDenied` on `PutBucketCors`. **2026-07-06:** retired the default Firebase Hosting URL from app allowlists; R2 CORS still needs Cloudflare access with a valid `CLOUDFLARE_API_TOKEN` to remove `https://your-project-id.web.app` and confirm `https://gen-fashion-app.web.app` is allowed. | §8.4, ADL-014 |
| MD-10 | Full vector seed to production | ✅ Implemented | **2026-06-28 (Milestone B complete):** `run_seed.py --with-embeddings` ran on ES VM against prod R2/ES/Firestore via ADC (`authorized_user`); result: `created=209, skipped=1, errors=0` (total_samples=210; `adult-01`/`adult-02`/`child-01` × 70 each); `clothing_items/_count=210`; embedding dim=768 confirmed on sample doc. `.env` emulator vars (`FIRESTORE_EMULATOR_HOST`, `GOOGLE_APPLICATION_CREDENTIALS`) were present on VM copy — commented out during seed. Completes M3-2 deployment portion. | §15 Phase 1a #7, §16.4, ADL-010 |
| MD-11 | Production Nano Banana image gen | ✅ Implemented | Production Firestore contains multiple `COMPLETED` sessions with `styleResult.modelUsed=gemini-2.5-flash-image` and R2 coordinate-image keys, including 2026-07-09 runs. The app no longer accepts `collage-fallback` as a completed coordinate. | §6.5, ADL-005 |
| MD-12 | Flutter Web build + Firebase Hosting | ✅ Implemented | `flutter build web --release` with production `--dart-define`s (`USE_EMULATORS=false`, real `FIREBASE_*`, `API_BASE_URL`=fastapi URL); deployed to Firebase Hosting. **2026-06-29 D.5:** `gen-fashion.web.app` was unavailable globally, so `gen-fashion-app.web.app` was created/deployed and returns 200. **2026-06-29 incident fix:** added `gen-fashion-app.web.app` to Firebase Auth authorized domains after Google sign-in failed with `unauthorized-domain`. New-origin R2 upload CORS still needs acceptance verification. | §11, ADL-025 |
| MD-13 | Production E2E smoke | ✅ Implemented | Production Firestore contains authenticated `SHARED_CLOSET` sessions that reached `COMPLETED` with `modelUsed=gemini-2.5-flash-image`, including a 2026-07-09 `STANDARD` run for `adult-01`. Live Cloud Run `/health` returns 200 and private ADK rejects unauthenticated access with 403. | §15 Phase 1a #6 |
| MD-14 | Logging, TTL & teardown | ✅ Implemented | Cloud Logging is queryable, Firestore TTL for `agentEvents.ttlAt` is ACTIVE, Artifact Registry keep-3 cleanup policy is configured, and `scripts/deploy/teardown.sh --project animation-agent --dry-run` lists the expected Cloud Run, Tasks, Artifact Registry, firewall, VM, static IP, schedule, secrets, and service-account resources. | §9.3, ADL-021 |
...
> **Progress update (2026-06-30): MF-1 ✅ complete; MF-2…MF-6 🟡 In progress.** Workload Identity Pool `github-pool`, OIDC provider `github-provider`, and `github-deployer` SA (roles: `run.admin`, `artifactregistry.writer`, `firebasehosting.admin`, `iam.serviceAccountUser` on each Cloud Run SA) are provisioned in project `your-project-id`. All 11 GitHub Actions secrets (`WIF_PROVIDER`, `WIF_SA`, ES/R2/Firebase config) added to the repo. `.github/workflows/ci-cd.yml` committed to `feat/dev-environment`; MF-2…MF-6 are in progress (workflow exists; first live CI run pending merge to `main`).
>
> **Status audit (2026-07-11):** GitHub Actions run `29016331719` (`main` push, commit `140cf69`) completed successfully on 2026-07-09. Jobs passed: `Test fastapi-service`, `Test adk-agent-service`, `Test flutter-web-app`, `Test deploy scripts`, and `Deploy to Production`. MF-2…MF-4 are now ✅. MF-5 remains 🟡 because the workflow currently runs `/health` smoke + rollback, while `req-phase01.md` §19.4 also requires an authenticated deployed coordination smoke to `COMPLETED`. MF-6 remains 🟡 because the operational commands live in `docs/gcp-cheatsheet.md`, but a dedicated pipeline runbook is still deferred.

| ID | Feature | Status | Description | Req ref |
|---|---|---|---|---|
| MF-1 | Workload Identity Federation | ✅ Implemented | Workload Identity Pool `github-pool` + OIDC provider `github-provider` created in project `your-project-id`; `github-deployer` SA provisioned with roles `run.admin` / `artifactregistry.writer` / `firebasehosting.admin` / `iam.serviceAccountUser`; WIF provider bound to SA by repo+ref. All 11 GitHub Actions secrets (`WIF_PROVIDER`, `WIF_SA`, ES/R2/Firebase) configured. Setup commands documented in `docs/gcp-cheatsheet.md`. **2026-06-30: operator-executed and verified.** | §19.1, ADL-030 |
| MF-2 | CI gate (tests + build) | ✅ Implemented | `.github/workflows/ci-cd.yml` runs parallel `test-fastapi` / `test-adk` / `test-flutter` / `test-deploy-scripts` jobs on push and PR. 2026-07-09 `main` push run `29016331719` passed all four test jobs. | §19.2, ADL-031 |
| MF-3 | CD — backend (Cloud Run) | ✅ Implemented | `Deploy to Production` builds/pushes both service images via Docker Buildx (with GHA cache), deploys Firestore indexes, then calls `scripts/deploy/deploy_adk.sh` + `deploy_fastapi.sh`. 2026-07-09 `main` run deployed successfully. | §19.3, ADL-032 |
| MF-4 | CD — frontend (Firebase Hosting) | ✅ Implemented | `Deploy to Production` builds Flutter Web with production `--dart-define`s and deploys Firebase Hosting via `scripts/deploy/deploy_firebase_hosting.py` using WIF-backed `gcloud auth print-access-token`. 2026-07-09 `main` run deployed successfully. | §19.3, ADL-032, ADL-025 |
| MF-5 | Post-deploy smoke + rollback | 🟡 In progress | `Deploy to Production` records the previous FastAPI revision, runs `GET /health → 200` after deploy, and rolls `fastapi-service` traffic back to the previous revision on smoke failure. 2026-07-09 `main` run passed that smoke step. **Remaining:** `req-phase01.md` §19.4 also requires an authenticated deployed coordination smoke to `COMPLETED`; this is not automated yet because CI has no Firebase ID-token path. | §19.4, ADL-032 |
| MF-6 | Pipeline runbook + matrix sync | 🟡 In progress | WIF setup commands and GitHub Secrets table in `docs/gcp-cheatsheet.md`; feature matrix kept in sync. Full pipeline runbook (dedicated doc) deferred to ExecPlan. | §19.5 |
