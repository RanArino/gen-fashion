...
> **Progress update (2026-06-30): MF-1 ✅ complete; MF-2…MF-6 🟡 In progress.** Workload Identity Pool `github-pool`, OIDC provider `github-provider`, and `github-deployer` SA (roles: `run.admin`, `artifactregistry.writer`, `firebasehosting.admin`, `iam.serviceAccountUser` on each Cloud Run SA) are provisioned in project `animation-agent`. All 11 GitHub Actions secrets (`WIF_PROVIDER`, `WIF_SA`, ES/R2/Firebase config) added to the repo. `.github/workflows/ci-cd.yml` committed to `feat/dev-environment`; MF-2…MF-6 are in progress (workflow exists; first live CI run pending merge to `main`).

| ID | Feature | Status | Description | Req ref |
|---|---|---|---|---|
| MF-1 | Workload Identity Federation | ✅ Implemented | Workload Identity Pool `github-pool` + OIDC provider `github-provider` created in project `animation-agent`; `github-deployer` SA provisioned with roles `run.admin` / `artifactregistry.writer` / `firebasehosting.admin` / `iam.serviceAccountUser`; WIF provider bound to SA by repo+ref. All 11 GitHub Actions secrets (`WIF_PROVIDER`, `WIF_SA`, ES/R2/Firebase) configured. Setup commands documented in `docs/gcp-cheatsheet.md`. **2026-06-30: operator-executed and verified.** | §19.1, ADL-030 |
