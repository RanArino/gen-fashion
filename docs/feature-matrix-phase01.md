...
> **Completion update (2026-06-30): MF-1…MF-6 → ✅ Implemented.** WIF pool/provider/SA provisioned; all 11 GitHub secrets set. First fully successful deploy run: **28444584922** (2026-06-30) — all three test jobs + Docker Buildx image push + Cloud Run ×2 deploy + Firebase Hosting REST API deploy + `/health` smoke check all ✅. Three deploy-path bugs fixed along the way: WIF `invalid_target` (pool path in secret), `firebase-tools` WIF incompatibility (replaced with `scripts/deploy/deploy_firebase_hosting.py` using `gcloud auth print-access-token`), REST API payload issues (headers list→map, `/populateFiles`→`:populateFiles`, SHA256 of raw→gzipped bytes). MF milestone complete.
...
| MF-2 | CI gate (tests + build) | ✅ Implemented | Parallel `test-fastapi` / `test-adk` / `test-flutter` jobs verified green on first live `main` push (run 28440536239, 2026-06-30). Branch protection on `main` and `develop` enforces these checks as a merge gate. | §19.2, ADL-031 |
| MF-3 | CD — backend (Cloud Run) | ✅ Implemented | `deploy` job: builds/pushes images via Docker Buildx (GHA cache), then calls `scripts/deploy/deploy_adk.sh` + `deploy_fastapi.sh`. Verified green in run 28444584922 (2026-06-30). | §19.3, ADL-032 |
| MF-4 | CD — frontend (Firebase Hosting) | ✅ Implemented | `deploy` job: `flutter build web --release` with production `--dart-define`s, then deploys via `scripts/deploy/deploy_firebase_hosting.py` (Firebase Hosting REST API + `gcloud auth print-access-token`, WIF-compatible). Verified green in run 28444584922 (2026-06-30). | §19.3, ADL-032, ADL-025 |
| MF-5 | Post-deploy smoke + rollback | ✅ Implemented | `deploy` job: `GET /health → 200` smoke after deploy; on failure rolls back `fastapi-service` to the pre-deploy revision via `gcloud run services update-traffic`. Smoke passed in run 28444584922 (2026-06-30). | §19.4, ADL-032 |
| MF-6 | Pipeline runbook + matrix sync | ✅ Implemented | WIF setup commands and GitHub Secrets table documented in `docs/gcp-cheatsheet.md`; `CONTRIBUTING.md` documents branch model and CI requirements; feature matrix kept in sync. | §19.5 |
