...
> **ExecPlan (2026-06-12):** [20260612-m5-coordination-flow-accordion-ui.md](plans/20260612-m5-coordination-flow-accordion-ui.md) covers the whole M5 milestone — M5-1…M5-11 — as one implementation slice. The selected path reuses the existing M2 closet upload pipeline, creates/updates sessions in FastAPI, starts the Python ADK service through direct HTTP after source selection (ADL-020), lets `adk-agent-service` write session state and `agentEvents` to Firestore (ADL-021), streams those events through FastAPI SSE (ADL-011), and renders them in the Flutter Accordion/result UI. Rows are 🟡 because an approved ExecPlan is now in flight; they remain non-implemented until the browser E2E passes.

| ID | Feature | Status | Description | Req ref |
|---|---|---|---|---|
| M5-1 | `CreateSessionUseCase` | 🟡 In progress | `POST /sessions` — creates `sessions/{id}` with `status: SOURCE_SELECTING`. Planned in the M5 ExecPlan. | §6.11, ADL-015 |
| M5-2 | `FirestoreStyleSessionRepository` | 🟡 In progress | Session state persistence + state machine transitions. Planned in the M5 ExecPlan. | §5.2, §8.1 |
| M5-3 | `SelectClothingSourceUseCase` | 🟡 In progress | Updates session `source`; validates `CLOSET` has data, `SHARED_CLOSET` always allowed. Planned in the M5 ExecPlan. | §6.2, §4.3 |
| M5-4 | `AnalyzeClothingImageUseCase` | 🟡 In progress | Analyzes image, saves result, transitions session to analyzed. Planned in the M5 ExecPlan. | §6.1 |
| M5-5 | `SearchCandidateItemsUseCase` | 🟡 In progress | Routes to closet/shared/Rakuten adapters; returns unified `CandidateItem` list. Planned in the M5 ExecPlan. | §6.3, ADL-008 |
| M5-6 | `GenerateCoordinateUseCase` | 🟡 In progress | Generates final coordinate image; session → `COMPLETED`. Planned in the M5 ExecPlan. | §6.5, §4.3 |
| M5-7 | Session lifecycle & timeout | 🟡 In progress | One image per session; new upload after `COMPLETED`/timeout → new session. Planned in the M5 ExecPlan. | §4.3, ADL-009 |
| M5-8 | ADK → Firestore event relay | 🟡 In progress | ADK writes `sessions/{id}/agentEvents/{eventId}`; event subcollection TTL. Planned in the M5 ExecPlan. | ADL-011 |
| M5-9 | SSE streaming endpoint | 🟡 In progress | `GET /sessions/{id}/stream` — FastAPI Firestore `on_snapshot` → SSE to Flutter. Planned in the M5 ExecPlan. | §15 Phase 1a #2, ADL-011 |
| M5-10 | Flutter Accordion UI | 🟡 In progress | Receives SSE, renders agent thinking steps as collapsible real-time UI. Planned in the M5 ExecPlan. | §11, §15 Phase 1a #3 |
| M5-11 | Web coordination flow E2E | 🟡 In progress | upload → agent thinking → candidates → select → image generated, fully working. Planned in the M5 ExecPlan. | §15 Phase 1a #6 |
