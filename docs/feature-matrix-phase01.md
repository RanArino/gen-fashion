...
| MF-2 | CI gate (tests + build) | 🟡 In progress | `.github/workflows/ci-cd.yml` implemented: parallel `test-fastapi` / `test-adk` / `test-flutter` jobs on every push + PRs targeting `main`/`develop`. Integration tests scoped to unit (no Emulator/ES service containers needed for current test suite). | §19.2, ADL-031 |
| MF-3 | CD — backend (Cloud Run) | 🟡 In progress | `deploy` job in `ci-cd.yml`: builds/pushes images via Docker Buildx (with GHA cache), then calls `scripts/deploy/deploy_adk.sh` + `deploy_fastapi.sh`; triggers on `main` push after all tests pass. | §19.3, ADL-032 |
| MF-4 | CD — frontend (Firebase Hosting) | 🟡 In progress | `deploy` job: `flutter build web --release` with production `--dart-define`s (secrets), then `firebase deploy --only hosting:gen-fashion-app` via ADC (WIF credential). | §19.3, ADL-032, ADL-025 |
| MF-5 | Post-deploy smoke + rollback | 🟡 In progress | `deploy` job: `GET /health → 200` smoke after deploy; on failure rolls back `fastapi-service` to previous revision via `gcloud run services update-traffic`. Full coordination smoke (m5_coordination_smoke.py) deferred — requires Firebase ID token, not automatable without a CI service account. | §19.4, ADL-032 |
